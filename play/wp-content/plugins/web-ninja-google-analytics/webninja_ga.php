...
define('wbgaversion', '1.0.7', true);
global $wbga_debug_enabled;
...
class GALib
{
  ...
  function GALib($auth, $ids = '', $cache_timeout = 360)
  {
    $this->auth = $auth;
    $this->ids = $ids;
...
add_action('admin_print_scripts', 'wbgad_admin_print_scripts');
function wbgad_admin_print_scripts() 
{
...
function wbga_get_option($option_name) {
	...
	if (!$wbga_options || !array_key_exists($option_name, $wbga_options)) {
		...
		$wbga_default_options['fb_cam_tracking'] 		= 0;
...
function wbga_options() {
  ...
  if (isset($_POST['info_update'])) {
	...
	$wbga_options['fb_cam_tracking'] 		= ($_POST['fb_cam_tracking']=="true"         ? true : false);
    update_option('webninja_ga_options', $wbga_options);
...
th {
    ...
    <fieldset class="options" name="tracking">
      ...
      <table width="500px" cellspacing="5" cellpadding="5" class="editform">
        ...
        <tr>
          <th nowrap valign="top">Facebook Ad Tracking: <span onmouseover="tooltip.show('This will add the extra code to the tracking javascript needed for Facebook Ads Campaign Tracking.', 400);" onmouseout="tooltip.hide();" style="color:#00F; cursor:pointer">[?]</span></th>
          <td><input type="checkbox" name="fb_cam_tracking" id="fb_cam_tracking" value="true" <?php if (wbga_get_option('fb_cam_tracking')) echo "checked"; ?> /></td>
		</tr>
        <tr>
...
function gad_admin_handle_other_options($info_message = '')
{
  ...
  $ga = new GALib(wbga_get_option('gad_auth_token'), '', wbga_get_option('gad_cache_timeout') !== false ? wbga_get_option('gad_cache_timeout') : 360);
...
{
?><input value="<?php echo (wbga_get_option('gad_cache_timeout') != "" ? wbga_get_option('gad_cache_timeout') : '360'); ?>" name="ga_cache_timeout" id="ga_cache_timeout"/><?php
}
else
{
...
";
if (wbga_get_option('fb_cam_tracking') == 1) {
$result .= "  _gaq.push(['_setCampSourceKey', 'utm_source']);
...
function gad_posts_pages_custom_column($column_name, $post_id, $post_or_page)
{
  ...
  {
    $ga = new GALib(wbga_get_option('gad_auth_token'), wbga_get_option('gad_account_id'), wbga_get_option('gad_cache_timeout') !== false ? wbga_get_option('gad_cache_timeout') : 360);
